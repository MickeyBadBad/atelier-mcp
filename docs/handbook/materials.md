...
### Surface imperfections — layered roughness + normal maps (cross-tutorial consensus)

Photorealism breaks when a surface reads as "perfect": a glossy plastic charger that's never seen a fingerprint, a uniformly-matte plaster wall, a brand-new metal fitting with mirror-clean roughness. Real surfaces have micro-history — fingerprints, smudges, dust, scuffs, scratches, edge wear — and the LLM-driven shader graphs that come out of `apply_archviz_material` start uniform by default. Layer in imperfection maps to fix this. Two of six surveyed photoreal interior tutorials apply this directly (Nuno Silva "7 Step Formula" lists "Surface Decals: dust, stains, and scratches break the CGI perfection" as Step 7; Nik Kottmann "How to add Surface Imperfections to Shaders for increased Realism!" walks the layered-map graph in detail — surveyed 2026-05).

Concrete recipe:
- **Roughness imperfections** (fingerprints, smudges, oils): mix a grayscale fingerprint/smudge map *into* the Roughness input via a **Mix Color** node in Screen blend mode. Texture set to **Non-Color** colorspace — this is essential; sRGB interpretation gives wrong roughness values and "blown out" highlights (per Blender Manual *Color Management* / *Non-Color Data*, Latest; corroborating Nik Kottmann's emphasis on this exact step).
- **ColorRamp compression on the imperfection map**: insert a ColorRamp before the Mix to compress the map's contrast — slide the black handle right, white handle left, pulling both toward mid-gray so the imperfection contributes a **0.3 - 0.7 roughness range**, not the raw 0 - 1 black/white values. Without this the surface flips between mirror-glossy and full-matte unrealistically (per Nik Kottmann; this also corroborates art_of_3d_rendering's single-source "drive Roughness with low-frequency noise to break perfectly-uniform surfaces" — both are different implementations of the same underlying principle: **break uniform roughness with mid-frequency variation**).
- **Normal/scratch imperfections** (physical scratches, micro-dents): a **separate** normal map mixed into the Principled BSDF Normal input. Mix strength low (0.1 - 0.3) — full strength reads as "etched glass", not "lightly used".
- **Box Mapping** when the geometry has no clean UV unwrap. Set the Image Texture's projection to **Box**, blend value 0.2 - 0.3, and use *Object* texture coordinates instead of UV. This avoids hard seams across complex geometry without manual UV work (per Blender Manual *Image Texture → Projection → Box*, Latest; cross-corroborated by Nik Kottmann's tutorial which uses this exact technique on a glossy black charger).

The Blender Manual documents the underlying primitives — Mix Color blend modes, Box Mapping projection, and Non-Color data space — as canonical Cycles workflows (per Blender Manual *Compositing → Color → Mix Node*, Latest; *Image Texture → Box Projection*, Latest; *Color Management → Non-Color Data*, Latest). **Cross-tutorial consensus: 2 of 6 surveyed (Nuno Silva + Nik Kottmann); plus partial corroboration of the underlying "break uniform roughness" principle from art_of_3d_rendering noise-driven roughness.** Resources for the imperfection maps themselves: cgbookcase.com (used by Nik Kottmann), Poliigon, [Surface Imperfections](https://www.surfaceimperfections.com/) — all return real-scanned grayscale fingerprint / scratch / dust map sets.

### Subtle micro-displacement on textiles for tactile realism (cross-tutorial consensus)

Fabrics are the surfaces where "perfectly flat CG" reads worst. A real cushion has gravity-induced sag; a sherpa throw has visible thread loops; a boucle armchair has bouclé loops that catch light at micro-angles. Two surveyed tutorials apply the same fix (coral_lab "Photorealistic Japandi Interior" notes "subtle displacement on sherpa/boucle for silhouette break"; Blender Guru "Using fabric textures in Blender (Couch Part 5)" calls these "subtle micro-creases via low-strength wrinkle map" — surveyed 2026-05). Two complementary tactics:

**1. Secondary bump / displacement at low strength.** Adds physical surface variation that breaks uniform appearance under raking light. Concretely: a Noise Texture (high frequency, scale 80 - 200) → **Bump** node at strength **0.05 - 0.15** → Principled BSDF Normal input. Strength matters: 0.5+ reads as "Marble countertop", not "fabric". The right range is "barely visible but you'd notice if it were gone" (per Blender Manual *Bump Node*, Latest; observed in both Blender Guru Couch Part 5 and coral_lab's sherpa-displacement application).

**2. Sheen on the Principled BSDF.** For fuzzy textiles (sherpa, boucle, velvet, mohair). Increase **Sheen Weight** to 0.5 - 1.0 and **Sheen Roughness** to 0.3 - 0.6 to capture the silhouette glow that real fuzzy fabrics produce at glancing angles (Fresnel effect). Without sheen, fuzzy fabrics render as "matte felt-board flat", losing the soft halo that distinguishes them from dense weave (per Blender Manual *Principled BSDF*, Latest, Sheen section — describes the Sheen layer as "useful for fabrics with fine fibers like velvet"; corroborated by Blender Guru's Couch Part 5 explicit Sheen tuning on the upholstery example).
